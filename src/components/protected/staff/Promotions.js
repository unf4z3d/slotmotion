      ...
      <div className="text-left">
        <span>
          {(row.active === undefined || row.active) && (
            <RaisedButton
              ...
              onClick={() => {
                this.handlePublishPromotion(row);
              }}
              className="btn-smotion secondary btn-min"
              label={row.active !== undefined ? 'Published' : 'Publish'}
              secondary
            />
          )}
        </span>
        ...
        <span>
          <RaisedButton
            ...
            onClick={() => {
              this.handleDeletePromotion(row);
            }}
      ...
      <div>
          ...
          <div className="text-right">
            <div>
              <RaisedButton
                ...
                onClick={() => this.setState({ promotionVisible: true })}
                type="submit"
                label="Create New"
              />
            </div>
            ...
            <div className="text-right">
              ...
              <span>
                <RaisedButton
                  ...
                  label={this.state.savingPromotion ? 'Saving Data' : 'Save Promotion'}
                  secondary
                />
              </span>
            </div>
        ...
        <div className="smotion-table">
          <BootstrapTable data={this.state.promotions} options={{ hideSizePerPage: true }} bordered={false}>
            <TableHeaderColumn width="100" dataField="name" isKey dataSort>
              Name
            </TableHeaderColumn>
            <TableHeaderColumn width="100" dataField="name" dataSort>
              Game
            </TableHeaderColumn>
            <TableHeaderColumn width="100" dataField="startDate" dataSort>
              Starts
            </TableHeaderColumn>
            <TableHeaderColumn width="100" dataField="endDate" dataSort>
              Ends
            </TableHeaderColumn>
            <TableHeaderColumn width="100" dataField="status" dataSort>
              Status
            </TableHeaderColumn>
            <TableHeaderColumn dataAlign="left" dataFormat={this.promotionActionsFormatter} />
          </BootstrapTable>
        </div>
      </div>
    );
